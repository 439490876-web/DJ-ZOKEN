...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen w-full bg-transparent flex flex-col text-slate-200 overflow-hidden font-sans app-shell">
      ...
      <div className="app-topbar macos-toolbar">
      ...
      {/* 左侧: 曲库面板 */}
      <GlassPanel tone="soft" className="w-[28%] min-w-[320px] max-w-[420px] flex flex-col macos-sidebar">
        <GlassCard className="p-4 z-10 flex flex-col gap-3">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full glass-input macos-textfield text-sm pl-10 pr-4 focus:outline-none focus:ring-1 focus:ring-dj-accent/40 transition-all placeholder-slate-500"
            />
          </div>
          ...
          {isFocusMode && (
              <GlassCard className="rounded px-2 py-1.5 flex items-center gap-2 text-[10px] text-slate-400 animate-in fade-in slide-in-from-top-1">
        ...
        >
            <GlassCard className={`mb-2 p-3 text-center text-xs transition-all macos-dropzone ${isLibraryDragOver ? 'is-active' : ''}`}>
                <div className="text-sm font-semibold text-slate-200">拖拽本地歌曲到这里</div>
                <div className="mt-1 text-[11px] text-slate-500">或</div>
                <button
                    onClick={() => fileInputRef.current?.click()}
                    className="mt-2 inline-flex items-center justify-center px-3 py-1 rounded-[10px] btn-secondary text-[11px] font-semibold"
                >
                    点击选择文件
                </button>
                <div className="text-[10px] text-slate-500 mt-1">仅本地导入，不会上传</div>
      ...
      {/* 中间: Set 编排区 */}
      <GlassPanel tone="soft" className="flex-1 flex flex-col relative">
        <div className="px-6 pt-4 pb-0 flex items-center justify-between">
            <SegmentedControl
                value={setType}
                onChange={(value) => setSetType(value as SetType)}
                options={[
                    { id: 'warmup', label: '暖场 (Warm-up)', icon: <Sunrise className="w-4 h-4" /> },
                    { id: 'prime', label: '黄金时段 (Prime)', icon: <Sun className="w-4 h-4" /> },
                    { id: 'closing', label: '收尾 (Closing)', icon: <Sunset className="w-4 h-4" /> },
                ]}
            />
        ...
        <div className="px-6 pt-3 pb-2 space-y-3">
            <div className="flex items-center gap-2">
                <input
                    ...
                    className="flex-1 glass-input macos-textfield text-sm placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-dj-accent/40"
                />
                <button
                    onClick={handleNewSet}
                    className="px-3 py-2 rounded-[10px] btn-secondary text-[12px] font-semibold transition-colors"
                >
                    新建 Set
                </button>
      ...
      {/* 右侧: 分析与 AI 面板 */}
      <GlassPanel tone="soft" className="w-[28%] min-w-[320px] max-w-[420px] flex flex-col">
        
        {/* 上半部分: 实时诊断与数据 */}
        <div className="flex flex-col shadow-xl z-10 max-h-[55%] min-h-[40%] resize-y overflow-hidden relative">
             <GlassCard className="px-4 py-3 flex items-center gap-2 sticky top-0 z-20 macos-section-header">
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-slate-950/30">
             ...
             <div className="flex-1 overflow-y-auto p-4 custom-scrollbar relative">
                 <GlassCard className="mb-3 rounded-md p-2">
                     ...
                     <div className="flex items-center gap-2">
                         <input
                             ...
                             className="flex-1 glass-input macos-textfield text-[11px] placeholder-slate-600 focus:outline-none focus:ring-1 focus:ring-dj-accent/40"
                             autoComplete="off"
                         />
                         <button
                 ...
                 <div className="space-y-2 pb-2">
                    {aiSuggestions.map((s, idx) => {
                       ...
                       return (
                           <GlassCard key={`${s.trackId}-${idx}`} className="p-2.5 rounded transition-all group relative animate-in fade-in slide-in-from-bottom-2 duration-300">
